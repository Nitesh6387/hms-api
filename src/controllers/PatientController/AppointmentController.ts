...
export const getPatientAppointmentByIdController = async (req: any, res: any) => {
  try {
...
export const cancelAppointmentController = async (req: any, res: any) => {
  try {
    const patientId = req.user.id;
    const { id } = req.params;
...
export const rescheduleAppointmentController = async (req: any, res: any) => {
  try {
    const patientId = req.user.id;
    const { id } = req.params;
    const { date, startTime } = req.body;
    if (!date || !startTime) return createResponse(res, 400, "New date and startTime required", [], false, true);
    ...
    if (conflict) {
      return createResponse(res, 409, "Doctor already has an appointment at that time", [], false, true);
    }
